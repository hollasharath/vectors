function createScreens(){
  ...
  function task_4_0_canvas(event){
  ...
  function task_4_1_canvas(event){
    if(event.touches.length == 2){
      touch_1 = event.touches[1];
      touch_2 = event.touches[1];
      if(touch_1.target.nodeName == "svg" && touch_2.target.nodeName == "svg"){
  ...
  parent_svg.on("touchstart", function(){
    if(currentScreen_count == 0){
      task_4_0_canvas(d3.event);
    }
    if(currentScreen_count == 1){
      task_4_1_canvas(d3.event);
    }
  })
