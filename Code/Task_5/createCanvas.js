function createCanvas(body){
  ...
  tasks_list = [];
  tasks_list[0] = "Create two vectors, add them and resolve the resultant vector.";
  tasks_list[1] = "Create two vectors, add them, resolve the resultant vector and recombine it.";
  ...
  title = parent_svg.append("text").styles({"font-size": 2*screen_size}).attrs({x: 0.5*body_width, y: 4*screen_size}).text("Addition using components");
  ...
  function help_slideshow(){
    ...
    var steps = [
      { imageUrl: '../../Images/vector_add_resolve.gif', imageWidth: '100%',
        html: '<span class="help_heading">Adding by components</span> <br> <span class="help_body">Resolve the resultant vector like any other vector.</span>',
